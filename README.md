...
- Public identity certificates are resolved lazily and cached for visible participant display. Direct messages use the peer's certified name and HTTPS avatar, with an abbreviated identity key and initials as safe fallbacks. The rail separates direct messages from groups; archive/favorite/mute state remains wallet-private, and active-message search runs only over locally decrypted content.
- Canonical mentions use `@<identity-key>` inside the encrypted message, render as the member's resolved name, and visually flag the mentioned recipient without making a display name part of authorization. The composer resolves suggestions only from the current private roster. Incoming audio and video meetings use distinct locally synthesized Convo ringtones.
