    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal-card details-modal" role="dialog" aria-modal="true" aria-labelledby="details-title">
        ...
        <div className="modal-content">
          {secret.kind === 'group' ? <><label className="field-label" htmlFor="private-group-title">Private group title</label><input id="private-group-title" name="private-group-title" className="text-field" disabled={!canAdmin} value={title} maxLength={100} onChange={(event) => setTitle(event.target.value)} /></> : <div className="resolved-direct-name"><span>Direct message with</span><strong>{conversationName(secret, identityKey, identityProfiles)}</strong><small>Resolved from their certified public identity, with their identity key used as the fallback.</small></div>}
